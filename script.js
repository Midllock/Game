...
function draw() {
    ...
    ctx.shadowBlur = 10;  // původně 40 → méně tlusté svícení
    ...
    ctx.beginPath();
    ctx.arc(centerX, centerY, gridSize * 0.5, 0, Math.PI * 2); // menší radius než 1.6
    ctx.fill();
    ctx.restore();
    ...
    ctx.rotate(time);
    ctx.beginPath();
    ctx.arc(0, 0, gridSize * 0.45, 0, Math.PI * 2); // mírně menší
    ...
    ctx.rotate(time * 3);
    ctx.beginPath();
    ctx.arc(0, 0, gridSize * 0.2, 0, Math.PI * 2);
    ...
    ctx.arc(
        centerX,
        centerY,
        gridSize * 0.1 + Math.sin(time * 4) * 3, // menší amplituda
        0,
        Math.PI * 2
    );
